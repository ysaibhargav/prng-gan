...
randthrs=0.0001 # the defined threshold for the tests
...
parser.add_argument('-t','--tests',nargs='*',type=int,default=[1,2,3,5,6,7,8,10,11,12,14,15],help='which tests to run, default: all')
...
def performrndtest(inputbits):
    ...
    for i in testlist:
         ...
         thistest=(eval("randtest."+tests.get(i))(inputbits))
         if args.evaluate:
...
args.outfile.write("\n".join(["%s => %s"%(_1, str(_2)) for _1, _2 in zip(header.split(), out)]) + "\n")
